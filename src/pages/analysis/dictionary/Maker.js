...
function MakerDictionary({dispatch, maker}) {
    ...
    const lastPageNum = maker['lastPageNum']||0
                    ...
                    <Grid container spacing={6}>
                        <Grid item xs={12}>
                            <DynamicTable dataList={
                                [
                                    { field: "아이디", data: (maker.hits || []).map(hit => ({id: hit.id, text: hit['sourceAsMap']['id']})) },
                                    { field: "키워드", data: (maker.hits || []).map(hit => ({id: hit.id, text: hit['sourceAsMap']['keyword']})) },
                                    { field: "값", data: (maker.hits || []).map(hit => ({id: hit.id, text: hit['sourceAsMap']['synonym']})) }
                                ]
                            }
...
export default connect(store => ({maker: store.dictionaryReducers.maker}))(MakerDictionary);
